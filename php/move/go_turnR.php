<?php
    ...
    // En allant vers haut et droite, il n'est possible d'atteindre que l'extrême nord ET est du labyrinthe. Si la case où le joueur souhaite se rendre n'est pas un mur, alors d'autres tests peuvent avoir lieu.
    // Vérifier que la case d'en haut est bien un couloir et pas un mur.
    if (($laby[$Z][$X - 1][$Y + 1] == 1 && $laby[$Z][$X - 1][$Y] == 1) || $X - 1 <= 0 || $Y + 1 >= 15) {
        echo "N_OK";
    } else {
        $newX = $X - 1;
        $newY = $Y + 1;
        ...
        if ($laby[$Z][$newX][$newY] == 4) {
            ...
            // N'accepter qu'un mouvement supplémentaire à droite à condition qu'il ne soit pas un mur extrême est.
        } elseif ($laby[$Z][$newX][$newY] == 2) {
            if ($laby[$Z][$newX][$newY] != 1) {
                if($laby[$Z][$newX][$newY+1]<15) {
                    $newY += 1;
                    $infoMoveJS = "pos+2";
